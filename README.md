...
`src/main.py` fetches each journal's source, parses it into `Article` objects, and serializes six RSS 2.0 files via `rfeed`. The strategy depends on what the publisher exposes:
...
- **RSS 1.0/RDF feed** (Cell, Molecular Cell, Cell Genomics, Trends in Genetics, Trends in Cell Biology, Science, Science Advances): Official RSS feeds are consumed directly. Cell-family research feeds filter by `prism:section` to keep only `Article`, `Short article`, and `Resource` (excluding Reviews, Perspectives, Editorials, Corrections, etc.). Science and Science Advances e-TOC feeds filter by `dc:type == "Research Article"`.
- **Nature per-subject listings** (Nature, Nature Communications): both journals are multidisciplinary, and roughly half of what they publish is outside this project's scope. About 65% of Nature Communications and 48% of Nature is biological or health science; the remainder is condensed-matter physics, astronomy, catalysis, atmospheric science, and machine learning. Neither journal's RSS feed nor its main article listing carries a subject anywhere, so `nature.com/subjects/<subject>/<journal>` is the only structural way to separate them. Both `biological-sciences` and `health-sciences` are fetched and unioned: Nature files cancer biology, immunology, and infection under health, so filtering on biology alone drops them. These pages predate the `c-card` layout used elsewhere and are parsed separately, keeping only cards explicitly typed `Article`, which also excludes `Matters Arising`, `Review Article`, and `Perspective`. No source declares an RSS fallback, because each journal's feed carries every subject and falling back to it would readmit exactly the content this filter removes.

  Each subject is fetched to a depth of `NATURE_SUBJECT_PAGES` pages. A rolling listing silently drops whatever scrolls off before the next snapshot, so its depth must exceed the longest gap between runs: at 50 cards per page, one page of biological sciences holds about three days of Nature Communications and eight of Nature, and the 20-item listing used previously lost 44 of the 88 Nature Communications articles published over one ten-day stretch. Two pages leave both journals headroom against a longest observed gap of six days.
...
This matters most on nature.com, where nine journals share one host and one card layout, so a mis-scoped response is otherwise indistinguishable from a correct one. On 2026-08-13 the Nature Reviews endpoints served a site-wide portfolio listing, and twenty Scientific Reports articles were published under the Nature Reviews Molecular Cell Biology label.

When the check empties a journal's results the generator falls back to that journal's own RSS endpoint where one is configured, reusing the existing fallback path. The subject-filtered sources deliberately declare none, because those feeds carry every subject. If a journal has no usable source left, it is skipped with an error and the remaining journals still publish, so one publisher's outage cannot freeze every feed. A feed that would come out empty is left unchanged rather than overwritten.
